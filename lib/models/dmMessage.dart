...
class dmMessage {
  ...
  static String newMessageinDm(String sender, String receiver){
    ...
    print("sender : ${newMessage.sender}, receiver : ${newMessage.receiver}, message : ${newMessage.message}");
    return messageJson;
  }
  ...
  static Future<void> sendMessage(String sender) async{
    ...
    databaseOp usersDb = databaseOp(path);
  ...
  static Future<void> showDm(String receiver) async{
    ...
    await usersDb.openDb();
    // int total = 10;
    // stdout.write("would you like to specify the number of messages to print?? [y/n] : ");
    // String? response = stdin.readLineSync();
    // if(response == 'y'){
    //   stdout.write("Please enter the number of messages you would like to see : ");
    //   total = int.tryParse(stdin.readLineSync()!) ?? 10;
    //   print(total);
    // }
    List<dynamic>? records = await usersDb.storeDb(true);
    String? receiverRec = await usersDb.findDb(receiver);
    ...
    for(int i=0; i<n; i++){
      ...
      // print('$i : $receiverMsg');
      if(receiverMsg!["sender"] == receiver && receiverMsg["receiver"] == sender){
        print("                      ${receiverMsg["message"]}  :$receiver");
      }
      else if(receiverMsg['receiver'] == receiver && receiverMsg["sender"] == sender && receiver!=sender){
